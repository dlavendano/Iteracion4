...
public class IngredientesServices {
	...
	public Response getIngredientes() {
		...
		try {
			Ingrediente = tm.darIngredientes();
		} catch (Exception e) {
	...
	@Path( "{nombre: ([a-z]|[A-Z])+}" )
	@Produces( { MediaType.APPLICATION_JSON } )
	public Response getIngredientes( @PathParam( "nombre" ) String nombre )
	{
		RotondAndesTM tm = new RotondAndesTM( getPath( ) );
		try
		{
			Ingrediente v = (Ingrediente) tm.buscarIngredientesPorNombre(nombre);
